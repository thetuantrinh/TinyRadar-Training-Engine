    ...
    for (i_batch = 0; i_batch < input_batches; i_batch++)
    {
        ...
        if (four_column_buf != four_column_buf_mid)
        {
            ...
            for (i = 0; i < output_ch; i++)
            {
                ...
                if (i == 0 && print_count < 10) {
                    printf("DEBUG_CONV1 pixel %d: raw_sum=%d, mult=%d, shift=%d, req_sum=%d, out_offset=%d\n", 
                           print_count, sum, output_mult[i], output_shift[i], arm_nn_requantize(sum, output_mult[i], output_shift[i]), out_offset);
                    print_count++;
                }
#endif
